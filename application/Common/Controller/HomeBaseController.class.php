...
/**
 * *
 * @var 通过用户uid查询用户角色balance by:2014.4.2 gl

		$id = session("id");
		if(!empty($id)){
			$id = M('Fx_user')->where(array('id'=>$id))->field('balance')->find();
        			$nav = M('Fx_list')->order(array("listorder" => "DESC"))->where(array('chown'=>$id['balance'],'parentid'=>0,'status'=>1))->select();
		        	foreach ($nav as $k => $v) {
		        		$nav["$k"]['url'] = "/index.php?g=&m=".$v['model']."&a=".$v['action']."&name=".$v['name'];
		        	}
    			$this->assign("nav", $nav);
		} */
/**
 * 
 * @var 通过角色ID查询菜单  by:2014.4.2 gl
 */
       /* $name = htmlspecialchars(I('get.name'));
        //查询所属ID
     	$navlist = M('Fx_list')->where(array('name'=>$name,'chown'=>$id['balance'],'status'=>1))->field('id,name')->select();
     	$vlist = M('Fx_list')->where(array('chown'=>$id['balance'],'status'=>1,'parentid'=>$navlist['0']['id']))->select();
        if(!empty($vlist)){
	        foreach ($vlist as $kl => $vl) {
	        	$vlist[$kl]['url'] = "/index.php?g=&m=".$vl['model']."&a=".$vl['action']."&name=".$vl['name'];
	        }
        }
